...
const LAYOUTS: Record<Exclude<ProjectGroup, 'client'>, Cell[]> = {
  ...
  // [                          | RAG (3)        ]
  product: [
    { slug: 'movingo', className: 'col-span-7 lg:col-span-4 lg:row-span-2' },
    { slug: 'vpdigital', className: 'col-span-7 md:col-span-4 lg:col-span-3' },
    { slug: 'rag', className: 'col-span-7 md:col-span-3 lg:col-span-3' },
  ],
  // [ Layers of the Earth (4) | post-bridge (3) ]
  own: [
    { slug: 'earthlayers', className: 'col-span-7 lg:col-span-4' },
    { slug: 'postbridge', className: 'col-span-7 lg:col-span-3' },
  ],
};
...
export default function ProjectSection({ group, locale, dict, projects }: Props) {
  ...
  return (
    <section
      ...
      ref={containerRef}
    >
      <p className="text-[16px] lg:text-[20px] text-muted text-center max-w-[640px] mx-auto">{text.description}</p>
      <motion.h2
        ref={h2Ref}
        style={{ opacity }}
        ...
        {LAYOUTS[group].map((cell, i) => {
          ...
          return (
            <motion.div
              ...
              className={`${cell.className} pt-4 border-t border-line`}
            >
